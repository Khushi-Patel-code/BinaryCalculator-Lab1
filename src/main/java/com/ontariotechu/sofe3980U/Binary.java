...
public class Binary
{
	...
	*/
	...
	public static Binary or(Binary b1, Binary b2) {
		int length = Math.max(b1.number.length(), b2.number.length());
		StringBuilder result = new StringBuilder();

		String s1 = String.format("%" + length + "s", b1.number).replace(' ', '0');
		String s2 = String.format("%" + length + "s", b2.number).replace(' ', '0');
	...
	public static Binary multiply(Binary num1, Binary num2) {
		...
		for (int i = multiplier.length() - 1; i >= 0; i--) {
			if (multiplier.charAt(i) == '1') {
				String shift = num1.number + "0".repeat(multiplier.length() - 1 - i);
